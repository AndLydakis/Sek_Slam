...
class AmclNode
{
  ...
  private:
    ...
    //ADDITIONS FOR PUBLISHING OPTIMAL SCAN
    ros::Publisher optimal_scan_pub_;
    //
...
{
    //ROS_INFO("DEBUG BEFORE MUTEX");
  boost::recursive_mutex::scoped_lock l(configuration_mutex_);
  //ROS_INFO("DEBUG AFTER MUTEX");
  ...
  //ADDITIONS FOR PUBLISHING OPTIMAL SCAN
    optimal_scan_pub_ = nh_.advertise<sensor_msgs::LaserScan>("optimal_scan", 2, true);
  //
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  else
  {
    ...
    ROS_INFO("Done initializing likelihood field model. dfsdhgsdhsh");
  }
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  //ADDITIONS FOR PUBLISHING OPTIMAL SCAN
  //ROS_INFO("DEBUG 4");
  bool opt_data_ = false;
  //AMCLLaserData //ldata_opt_;
  AMCLLaserData ldata;
  //
  ...
  if(lasers_update_[laser_index])
  {
    ...
    ////ldata_opt_.range_count = laser_scan->ranges.size();
    ...
    if(laser_max_range_ > 0.0)
    {
      ...
      ////ldata_opt_.range_max = std::min(laser_scan->range_max, (float)laser_max_range_);
    }
    else
    {
      ldata.range_max = laser_scan->range_max;
      ////ldata_opt_.range_max = laser_scan->range_max;
    }
    double range_min;
    ...
    ////ldata_opt_.ranges = new double[//ldata_opt_.range_count][2];
    ROS_ASSERT(ldata.ranges);
    //ROS_ASSERT(//ldata_opt_.ranges);
    for(int i=0;i<ldata.range_count;i++)
    {
      ...
      if(laser_scan->ranges[i] <= range_min)
      {
        ...
        ////ldata_opt_.ranges[i][0] = //ldata_opt_.range_max;
      }
      else
      {
        ldata.ranges[i][0] = laser_scan->ranges[i];
        ////ldata_opt_.ranges[i][0] = laser_scan->ranges[i];
      }
      // Compute bearing
      ldata.ranges[i][1] = angle_min +
              (i * angle_increment);
      ////ldata_opt_.ranges[i][1] = angle_min +
      //       (i * angle_increment);        
    }

    lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&ldata);
    //lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&//ldata_opt_);
    ...
    if(!(++resample_count_ % resample_interval_))
    {
      ...
      resampled = true;
      opt_data_ = true;
      //ldata_opt_.ranges = ldata.ranges;
      //ldata_opt_.sensor = lasers_[laser_index];
      //ldata_opt_.range_count = laser_scan->ranges.size();
    }
    //ROS_INFO("DEBUG 6");
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      {
          /*
          double optimal_map_range_;
          double optimal_obs_range_, optimal_obs_bearing_;
          
          pf_vector_t hit;
          pf_vector_t pose;
          AMCLLaser *self = (AMCLLaser*) //ldata_opt_.sensor;
          int optimal_step_ = (//ldata_opt_->range_count - 1) / (self->max_beams - 1);
             */
          //lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&//ldata_opt_);
          sensor_msgs::LaserScan optimal_scan_ ;
          std::vector<double> optimal_scan_ranges;
          std::vector<float>::iterator it; 
          double x_opt = hyps[max_weight_hyp].pf_pose_mean.v[0];
          double y_opt = hyps[max_weight_hyp].pf_pose_mean.v[1];
          double z_opt = hyps[max_weight_hyp].pf_pose_mean.v[2];
          //ROS_INFO("DEBUG 8");
          optimal_scan_ranges = lasers_[laser_index]->GetOptimalScan((AMCLLaserData*)&/*ldata_opt_*/ldata, (double)x_opt, (double)y_opt, (double)z_opt);
          std::vector<float> v_float(optimal_scan_ranges.begin(), optimal_scan_ranges.end());
          //ROS_INFO("DEBUG 9");
          //for(it=v_float.begin() ; it < v_float.end(); it++) 
          //{
          optimal_scan_.ranges = v_float;
          //}
          optimal_scan_.header.frame_id = "/laser";
          optimal_scan_.header.stamp = ros::Time::now();
          optimal_scan_pub_.publish(optimal_scan_);

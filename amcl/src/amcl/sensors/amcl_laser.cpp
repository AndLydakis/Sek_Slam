...
std::vector<double> AMCLLaser::GetOptimalScan(AMCLLaserData *data, double x, double y, double z)
{
    AMCLLaser *self;
    int i, j, step;
    double p;
    ...
    self = (AMCLLaser*) data->sensor;
    std::vector<double> optimal_scan;
    //std::vector<double> *optimal_scan = new std::vector<double>();
    ...
    {
      //printf("dfghsdhfsdhsd");
      obs_range = data->ranges[i][0];
      obs_bearing = data->ranges[i][1];
      
      map_range = map_calc_range(self->map, pose.v[0], pose.v[1],
                                 pose.v[2] + obs_bearing, data->range_max);
      optimal_scan.push_back(map_range);
...
/*
static double getSigmaHit(AMCLLaserData *data)
{
    AMCLLaser *self;
    self = (AMCLLaser*) data->sensor;
